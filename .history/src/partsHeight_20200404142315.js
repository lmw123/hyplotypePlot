 ...
 * @LastEditTime: 2020-04-04 14:23:15
 */
...
export const linkHighlight = (allNodes, allLinks, link, opacity) => {
    ...
    allNodes.each(function(p) {
        var item;
        item = d3.select(this);
        item.style("opacity", opacity);

        if(link.source.id === p.id || link.target.id === p.id)
        {
            item.style("opacity", "1");
            console.log("iuihiogo");
        }

    });

   // allNodes.style('stroke-opacity', o => (link.source.id === o.id || link.target.id === o.id ? 1 : opacity));
